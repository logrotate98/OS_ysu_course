...
class ChopStick(object):
    ...
    def __init__(self, number):
        self.number = number           # stick ID
        self.user = -1                 # keep track of philosopher using it
        self.lock = threading.Condition(threading.Lock())
        self.taken = False

    def take(self, user):         # for synchronization
        with self.lock:
    ...
    def drop(self, user):         # for synchronization
        with self.lock:
...
class Philosopher (threading.Thread):
    ...
    def __init__(self, number, left, right, butler):
        ...
        self.number = number            # philosopher id
        self.left = left
        self.right = right
        self.butler = butler
